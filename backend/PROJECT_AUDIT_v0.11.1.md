# Sadhana Card Tracker — Consolidated Project Audit v0.11.1
...
## v0.11.1 runtime stabilization

The first real-PostgreSQL monthly smoke run exposed an ORM/model-name mismatch in four-week response construction: `DevoteeCategory` stores its human-readable label in `display_name`, while the monthly service attempted to read a nonexistent `name` attribute. The service now uses `display_name`. A regression test now constructs a report using real SQLAlchemy mapped models and JSON-serializes the Pydantic response, so this mismatch is covered by the automated suite.
